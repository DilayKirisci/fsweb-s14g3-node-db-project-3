...
async function findById(scheme_id) {
	const schemeSteps = await db("scheme as sc")
	...
	const response = {
		scheme_id: parseInt(scheme_id, 10),
		scheme_name: schemeSteps[0],
		steps: [],
	};
	if (schemeSteps.length === 0) {
		return response;
	}
	for (let i = 0; i < schemeSteps.length; i++) {
		const item = schemeSteps[i];
		let stepModel = {
			step_id: item.step_id,
			step_number: item.step_number,
			instructions: item.instructions,
		};
		response.push(stepModel);
	}
	return response;
	// Egzersiz B
	/*
...
async function add(scheme) {
  ...
  */
	const [scheme_id] = await db("schemes").insert(scheme); // insert otomatic id veriyor
